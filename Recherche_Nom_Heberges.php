...
$tab_variables = array('annuler','Horigine', 'Nom', 'Phon');
...
if ($res = lect_sql($sql)) {
	...
	while ($enr_site = $res->fetch(PDO::FETCH_NUM)) 
		...
		$sql2 = 'select nom, prenoms, reference, Ne_le, Decede_Le from sg_'.$enr_site[0].'_personnes '.$req2;
		if ($res2 = send_sql($db_cible,$sql2)) {
			while ($enr2 = $res2->fetch(PDO::FETCH_NUM)) 
				...
				echo '<td><a href="http://genealogies.geneamania.net/'.$enr_site[1].'/Fiche_Fam_Pers.php?Refer='.$enr2[2].'">'.
					$enr2[1].'&nbsp;'.$enr2[0].$Dates.'</a></td></tr>'."\n";
			}
		}
  	}
}
