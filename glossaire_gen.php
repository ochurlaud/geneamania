...
$titre = 'Glossaire de généalogie';       // Titre pour META
$x = Lit_Env();
...
function Lettre($lettre) {
	...
	echo '&nbsp;&nbsp;<img id="ajout'.$lettre.'" src="'.$chemin_images.$Icones['oeil'].'" alt="Fl&egrave;che"'.Survole_Clic_Div('div'.$lettre).'/>'."\n";
	echo '</td></tr></table>'."\n";
	echo '<div id="div'.$lettre.'"><table width="95%" border="0" cellspacing="1" cellpadding="3" align="center">'."\n";
	return 0;
}

// Affiche la ligne avec le mt
function Mot($mot,$Ancre='',$premier=0) {
	global $num_lig, $def_enc;
	if (!$premier) echo '</tr>';
	...
	echo '<b>'.htmlentities($mot, ENT_QUOTES, $def_enc).'</b></td>'."\n";
}

$compl = Ajoute_Page_Info(600,150);
Insere_Haut(htmlentities($titre, ENT_QUOTES, $def_enc),$compl,'glossaire_gen','');
...
echo $ent_table.'<tr>'.
	'<td colspan="26" align="center">'.Affiche_Icone('tip','Conseil').'&nbsp;Cliquez sur les mots en <font color="blue">bleu</font>, ils
		renvoient &agrave; un autre mot ou sur <img src="'.$chemin_images.$Icones['loupe'].'" border="0" alt="En savoir plus"/> pour avoir plus de d&eacute;tails.
...
<?php $x = Mot('Cognat - Cognatique','CO'); ?>
<td>
...
<?php $x = Mot('Dispenses d\'affinité','dispenseA'); ?>
<td>
...
<?php $x = Mot('Registres paroissiaux','RE'); ?>
<td>
